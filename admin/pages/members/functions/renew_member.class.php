...
class MemberRegistration {
    ...
    public function renewMember($data) {
        try {
            ...
            if (!$memberId) {
                throw new Exception("Member ID is required for renewal");
            }
            ...
            try {
                ...
                if (!empty($data['membership_plan'])) {
                    error_log("Processing membership plan");
                    try {
                        // Get membership plan details
                        $planDetails = $this->getMembershipPlanDetails($data['membership_plan']);
                        if (!$planDetails) {
                ...
                if (!empty($data['selected_programs'])) {
                    error_log("Processing program subscriptions");
                    try {
                        $selectedPrograms = json_decode($data['selected_programs'], true);
                        error_log("Selected programs: " . print_r($selectedPrograms, true));
                ...
                // Handle rental services
                if (!empty($data['rental_services'])) {
                    error_log("Processing rental services");
                    try {
                        foreach ($data['rental_services'] as $rentalId) {
                            $rentalDetails = $this->getRentalServiceDetails($rentalId);
                            if (!$rentalDetails) {
                ...
                $this->pdo->commit();
                error_log("Transaction committed successfully");
                error_log("=== END MEMBER RENEWAL DEBUG ===\n");
                ...
                $this->pdo->rollBack();
                error_log("ERROR: Transaction rolled back. Error: " . $e->getMessage());
                error_log("=== END MEMBER RENEWAL DEBUG ===\n");
                throw $e;
            }
        } catch (Exception $e) {
