$(document).ready(function() {
    ...
    $(document).on('click', '.remove-program', function() {
        ...
        if (index >= 0 && index < selectedPrograms.length) {
            ...
            if (program.id) {
                ...
                if (schedule) {
                    selectedSchedules.set(program.id, schedule.filter(slot => slot.startTime !== program.startTime || slot.endTime !== program.endTime));
    ...
    function updateReviewInformation() {
        ...
        $('.review-registration-fee').text('₱ ' + window.registrationFee.toFixed(2));
        updateTotalAmount();
    }
