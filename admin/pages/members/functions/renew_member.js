...
$(document).ready(function () {
    ...
    $(document).on('click', '#renewSubmitBtn', function() {
        ...
        var programsWithSchedules = (selectedPrograms || []).map(function(program) {
            // Only generate schedules if all needed fields are present
            if (program && program.day && program.startTime && program.endTime && program.price && startDate && endDate) {
    ...
    function updateReviewInformation() {
        ...
        $('input[name="rental_services[]"]:checked').each(function() {
            ...
            rentalsHtml += `<div class="rental-item review-rental mb-2" data-rental-id="${$(this).val()}">
        ...
        // Attach remove handler for review phase
        $('.remove-rental-review').off('click').on('click', function() {
    ...
    function updateProgramsSummary() {
        ...
        selectedPrograms.forEach((program, index) => {
            ...
            if (plan.length && startDate) {
                ...
                const endDate = calculateEndDate(startDate, duration, durationType);
                numSessions = generateProgramDates(startDate, endDate, program.day, program.startTime, program.endTime, program.price).length;
            }
            const total = program.price * numSessions;
            totalProgramsFee += total;
            html += `
                <div class="summary-row mb-2" data-index="${index}" data-program-id="${program.id}">
        ...
        $('.remove-program').off('click').on('click', function(e) {
            e.preventDefault();
            const index = $(this).closest('.summary-row, .program-item').data('index');
            if (typeof index !== 'undefined') {
                selectedPrograms.splice(index, 1);
                updateProgramsSummary();
                updateReviewInformation();
...
function generateProgramDates(startDate, endDate, dayOfWeek, startTime, endTime, price, programType = null, programId = null) {
    const schedules = [];
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        if (d.getDay() === targetDay) {
            const scheduleObj = {
                ...
                coach_personal_schedule_id: (programType === 'personal') ? programId : null
            };
    ...
    $(document).on('click', '.select-schedule', function(e) {
        ...
        const scheduleData = {
            ...
            coach_group_schedule_id: button.data('type') === 'group' ? button.data('id') : null,
            coach_personal_schedule_id: button.data('type') === 'personal' ? button.data('id') : null
        };
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data && response.data.length > 0) {
                    ...
                    programDesc.text(`${programName} (${response.program_type_description || ''})`);
                } else if (response.message) {
                    tableBody.html(`<tr><td colspan="6" class="text-center">${response.message}</td></tr>`);
    ...
    $('input[name="membership_plan"], .rental-service-checkbox').on('change', function() {
        updateRentalServicesSummary();
        updateTotalAmount();
    });
    ...
    function updateRentalServicesSummary() {
        ...
        // Attach remove handler for summary section
        $('.remove-rental').off('click').on('click', function() {
            const rentalId = $(this).closest('.summary-row, .rental-item').data('rental-id');
    ...
    $('#renewMemberForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    alert('Membership renewed successfully!');
                    window.location.href = BASE_URL + '/admin/pages/members/members_new.php';
                } else {
