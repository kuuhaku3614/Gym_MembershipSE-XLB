...
try {
    ...
    $deleted = $validator->deletePendingWalkinTransaction($user_id, $transaction_id); // Use the new function

    if ($deleted) {
        echo json_encode(['success' => true, 'message' => 'Pending walk-in deleted successfully.']);
    } else {
        ...
        echo json_encode(['success' => false, 'message' => 'Failed to delete pending walk-in. It might have been processed already or does not exist.']);
    }
} catch (Exception $e) {
    error_log("Server error deleting pending walk-in: " . $e->getMessage());
