...
function getFilteredData($pdo, $start_date, $end_date) {
    ...
    // Add similar queries for other data types with date filtering
    // Earnings
    $earnings_sql = "
        SELECT 
            MONTH(created_at) as month,
    ...
    $utilization_sql = "
        ...
        JOIN personal_details pd ON u.id = pd.user_id
        LEFT JOIN transactions t ON u.id = t.user_id
        LEFT JOIN memberships m ON t.id = m.transaction_id AND m.created_at BETWEEN :start_date AND :end_date
        LEFT JOIN program_subscriptions ps ON t.id = ps.transaction_id AND ps.created_at BETWEEN :start_date AND :end_date
        LEFT JOIN rental_subscriptions rs ON t.id = rs.transaction_id AND rs.created_at BETWEEN :start_date AND :end_date
        JOIN roles r ON u.role_id = r.id
        WHERE r.role_name = 'member'
        GROUP BY u.id
        ORDER BY membership_count DESC";
    $utilization_stmt = $pdo->prepare($utilization_sql);
    $utilization_stmt->bindParam(':start_date', $start_date);
    $utilization_stmt->bindParam(':end_date', $end_date);
    $utilization_stmt->execute();
    ...
    // Calculate summary metrics for the date range
    $total_members_sql = "SELECT COUNT(DISTINCT id) as total FROM users";
    $total_members_stmt = $pdo->prepare($total_members_sql);
    $total_members_stmt->execute();
    ...
    $total_revenue_sql = "
        SELECT 
            (SELECT COALESCE(SUM(amount), 0) FROM memberships WHERE created_at BETWEEN :start_date AND :end_date) +
            (SELECT COALESCE(SUM(amount), 0) FROM program_subscriptions WHERE created_at BETWEEN :start_date AND :end_date) +
            (SELECT COALESCE(SUM(amount), 0) FROM rental_subscriptions WHERE created_at BETWEEN :start_date AND :end_date) as total_revenue";
    $total_revenue_stmt = $pdo->prepare($total_revenue_sql);
    $total_revenue_stmt->bindParam(':start_date', $start_date);
    $total_revenue_stmt->bindParam(':end_date', $end_date);
    $total_revenue_stmt->execute();
...
function generatePreviewHTML($data, $start_date, $end_date, $selected_sections) {
    ...
    <body>
        <div class="report-header">
            ...
            <p class="report-period">Period: <?= date('F d, Y', strtotime($start_date)) ?> - <?= date('F d, Y', strtotime($end_date)) ?></p>
        ...
        <div class="stats-container">
            <div class="stat-box">
                ...
                <div class="stat-value"><?= number_format($data['total_members']) ?></div>
            </div>
            <div class="stat-box">
            ...
            <div class="stat-box">
                <div class="stat-title">Average Check-ins</div>
                <div class="stat-value"><?= number_format($data['avg_checkins'], 1) ?></div>
            </div>
        </div>
        
        <?php if (in_array('attendance', $selected_sections) && !empty($data['attendance'])): ?>
        <div class="section">
            <h2 class="section-title">Member Attendance Analysis</h2>
            <table>
                <thead>
                    <tr>
                        <th>Username</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= number_format(($row['total_check_ins'] / max(1, $row['total_check_ins'] + $row['total_missed'])) * 100, 1) ?>%</td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <?php if (in_array('earnings', $selected_sections) && !empty($data['earnings'])): ?>
        <div class="section page-break">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($data['earnings'] as $row): ?>
                    <tr>
        ...
        <?php if (in_array('utilization', $selected_sections) && !empty($data['utilization'])): ?>
        <div class="section page-break">
            <h2 class="section-title">Member Service Utilization</h2>
            <table>
                <thead>
                    <tr>
                        <th>Username</th>
                ...
                <tbody>
                    <?php foreach ($data['utilization'] as $row): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['username']) ?></td>
                        <td><?= htmlspecialchars($row['first_name']) ?></td>
                        <td><?= htmlspecialchars($row['last_name']) ?></td>
        ...
        <?php if (in_array('programs', $selected_sections) && !empty($data['programs'])): ?>
        <div class="section page-break">
            <h2 class="section-title">Program Subscriptions</h2>
            <table>
                <thead>
                    <tr>
                        <th>Month</th>
        ...
        <?php if (in_array('rentals', $selected_sections) && !empty($data['rentals'])): ?>
        <div class="section page-break">
            <h2 class="section-title">Rental Subscriptions</h2>
            <table>
                <thead>
                    <tr>
                        <th>Month</th>
                ...
                <tbody>
                    <?php foreach ($data['rentals'] as $row): ?>
                    <tr>
                        <td><?= date('F', mktime(0, 0, 0, $row['month'], 10)) ?></td>
                        <td><?= $row['year'] ?></td>
        ...
        <div class="footer">
            <p>© <?= date('Y') ?> Gym Management System. All rights reserved.</p>
        </div>
    </body>
    </html>
    <?php
...
// Add this code to handle PDF export
use Dompdf\Dompdf;
use Dompdf\Options;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['export_pdf'])) {
    ...
    $html = generatePreviewHTML($data, $start_date, $end_date, $selected_sections);
    ...
    $dompdf->stream("analytics_report_" . date('Y-m-d') . ".pdf", array("Attachment" => true));
    exit();
}
?>
...
$('#export_pdf_btn').on('click', function() {
    ...
    // Create a new form for PDF submission
    ...
    selectedSections.forEach(function(section) {
        ...
        sectionInput.name = 'report_sections[]';
        sectionInput.value = section;
        form.appendChild(sectionInput);
    });
    
    // Add export_pdf field
